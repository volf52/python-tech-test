...
from ushort.shortener import is_valid_url, validate_url_id


@pytest.mark.parametrize(
    "url_id", ["u8Iv7Rc", "BD50BCq", "KCpn0KU", "I5b4NCH", "eY4FkUG"]
)
def test_validate_url_id_with_valid_ids(url_id: str):
    assert validate_url_id(url_id) is None


@pytest.mark.parametrize(
    "url_id", ["u8Iv7Rcq", "BD550BCq", "KCpnp0KU", "I5b4NCasadH", "eFkUG", ""]
)
def test_validate_url_id_with_invalid_length_ids(url_id: str):
    assert validate_url_id(url_id) == "Invalid length for the URL ID"


def generate_random_id_with_punct():
    cuttoff = random.randint(0, 6)
    chars = random.choices(ascii_letters + digits, k=cuttoff) + random.choices(
        punctuation, k=7 - cuttoff
    )
...
@pytest.mark.parametrize("url_id", [generate_random_id_with_punct() for _ in range(10)])
def test_validate_url_id_with_invalid_character_ids(url_id: str):
    assert validate_url_id(url_id) == "URL ID has some invalid characters"
...
def test_returns_failed_validation_on_invalid_url(address):
    assert not is_valid_url(address)
